...
import com.willblaschko.android.alexa.templateruntime.RenderTemplateItem;
import com.willblaschko.android.alexavoicelibrary.actions.BaseListenerFragment;
import com.willblaschko.android.alexavoicelibrary.actions.ShowDisplayFragment;
...
public abstract class BaseActivity extends AppCompatActivity implements BaseListenerFragment.AvsListenerInterface{
    ...
    private List<AvsItem> avsDisplayQueue = new ArrayList<AvsItem>();
    ...
    private void handleResponse(AvsResponse response){
        ...
        if(response != null){
            ...
            for(int i = response.size() - 1; i >= 0; i--){
                ...
                if (response.get(i) instanceof RenderTemplateItem){
                    avsDisplayQueue.add(response.get(i));
                    response.remove(i);
                }
            }
    ...
    private void checkQueue() {
        ...
        if (current instanceof AvsPlayRemoteItem) {
            //play a URL
            if (avsDisplayQueue != null && avsDisplayQueue.size() > 0 && avsDisplayQueue.get(0) != null && avsDisplayQueue.get(0) instanceof RenderTemplateItem) {
                Gson gson = new Gson();
                String json = gson.toJson(((RenderTemplateItem) avsDisplayQueue.get(0)).getPayload());
                Log.i("AvsHybridHelper", "render temp- " + json);
                try {
                    setDisplayMetadata(new JSONObject(json));
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                avsDisplayQueue.remove(0);
            }
    ...
    private void setDisplayMetadata(JSONObject json) {
        if (json == null){
            json = new JSONObject();
        }
                ...
                if (json.has("title")){
                    try{
                        ...
                        if (title.has("mainTitle")){
                            String titlebody = json.getString("textField");
                            String mainTitle = title.getString("mainTitle");
                            Log.d("mainTitle", mainTitle);
                            Log.d("Title", titlebody);

                            ShowDisplayFragment ldf = new ShowDisplayFragment ();
                            Bundle args = new Bundle();
                            args.putString("title", mainTitle);
                            args.putString("body", titlebody);
                            ldf.setArguments(args);
                            //Inflate the fragment
                            getFragmentManager().beginTransaction().add(R.id.textfrag, ldf).commit();
